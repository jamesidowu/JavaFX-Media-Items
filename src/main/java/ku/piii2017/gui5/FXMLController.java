...
//        label.setText("Hello World!");
    ...
    // Question 3: Initiation of method for two TableViews in GUI
    @FXML
    private void getAllButNoDups1(ActionEvent event) {
    getAllButNoDups(tableView1);
}
    
    @FXML
    private void getAllButNoDups2(ActionEvent event) {
    getAllButNoDups(tableView2);
}
    ...
    // Code for Get All But No Duplicates method; TableView as parameter
    private void getAllButNoDups(TableView<MediaItem> tableView) {
    
    Set<MediaItem> tableData = new HashSet<>(tableView.getItems());
    DuplicateFinder d = new DuplicateFindFromID3();
    Set<MediaItem> allButNoDups = d.getAllButNoDuplicates(tableData);
    
    
      ObservableList<MediaItem> dataForTableViewAndModel
                = FXCollections.observableArrayList(allButNoDups);
